...
bool GUI::initialize(const AppConfig& config)
{
    ...
    windowWidth = DPI(config.windowWidth);
    windowHeight = DPI(config.windowHeight);
    window = glfwCreateWindow(windowWidth, windowHeight, config.windowTitle.c_str(), nullptr, nullptr);
    if (window == nullptr)
    {
        return false;
    }
    ...
    if (config.enableIconFont && !config.iconFontPath.empty())
    {
        ...
        fonts["Icon"] = io.Fonts->AddFontFromFileTTF(config.iconFontPath.c_str(), iconFontSize, &icons_config, icons_ranges);
    }

    for (const FontSpec& font : config.fonts)
    {
        if (!font.name.empty() && !font.path.empty())
        {
            fonts[font.name] = io.Fonts->AddFontFromFileTTF(font.path.c_str(), font.size);
        }
    }

    // Setup style
    setupStyle();
    if (config.styleCallback)
    {
    ...
    // Window icon
    {
        const char* iconPath = config.windowIconPath.empty() ? nullptr : config.windowIconPath.c_str();

        GLFWimage images[1];
        images[0].pixels = iconPath ? stbi_load(iconPath, &images[0].width, &images[0].height, 0, 4) : nullptr; // RGBA
...
bool GUI::beginFrame(bool pollEvents)
{
...
ImFont* GUI::getFont(const std::string& fontName)
{
